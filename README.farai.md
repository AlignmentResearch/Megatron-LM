...
| 4 | Frozen torch grouped GEMM and frozen-input recompute guard — pending in [PR #3](https://github.com/AlignmentResearch/Megatron-LM/pull/3) | `megatron/core/` |
...
context `fork-base`. `tools/fork_base.py` is shared with the other FAR.AI forks; port behavior changes across the
forks so their compatibility checks stay aligned.
