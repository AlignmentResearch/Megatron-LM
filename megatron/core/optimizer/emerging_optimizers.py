...
class TensorParallelMuon(OrthogonalizedOptimizer):
    ...
    def scaled_orthogonalize_fn_with_gtp_remat(self, p, grad, tp_group, partition_dim):
        ...
        if mode == "blockwise":
            ...
            return self.scaled_orthogonalize_fn(grad, tp_group, None, tp_mode_this_group=mode)
        ...
        if mode == "duplicated":
            ...
            gathered_grad = self.scaled_orthogonalize_fn(
                gathered_grad, tp_group, partition_dim, tp_mode_this_group=mode
            )
