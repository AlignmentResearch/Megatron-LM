...
class _TorchGroupedFusedLoRA(torch.autograd.Function):
    ...
    def forward(
        ...
        augmented_output = _run_torch_grouped_mm(
    ...
    def backward(ctx, grad_output: Tensor):
        ...
        grad_x = _run_torch_grouped_mm(
            grad_augmented_output, ctx.augmented_weight, offsets
        )
            ...
            if not self._torch_grouped_weight_layout_is_current():
                ...
                if '_torch_grouped_weight' in self._buffers:
                    ...
                    self.register_buffer(
                        '_torch_grouped_weight', grouped_weight, persistent=False
                    )
        ...
        def _torch_grouped_mm_forward(self, x, m_splits):
            ...
            if input_rows == 0:
                ...
                output = torch._grouped_mm(
                    x_2d, grouped_weight.transpose(1, 2), offs=offsets
                )
            return output.reshape(*x.shape[:-1], output.shape[-1]), None
